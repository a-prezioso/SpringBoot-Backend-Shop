...
//	}
	...
	{
		mockMvc.perform(MockMvcRequestBuilders.get("/api/utenti/cerca/userid/Alex")
				.accept(MediaType.APPLICATION_JSON))
				...
				.andExpect(jsonPath("$.userId").value("Alex"))
				...
				assertThat(passwordEncoder.matches("Alex1993!", 
						utentiRepository.findByUserId("Alex").getPassword()))
				.isEqualTo(true);
	}
	...
	public void D_testGetAllUser() throws Exception
	{
		mockMvc.perform(MockMvcRequestBuilders.get("/api/utenti/cerca/tutti")
				...
				.andExpect(jsonPath("$[0].userId").value("Alex"))
